...
class EVOProviderIPCheck
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        $isAllowed = ProviderIPWhitelist::where('provider_code', GamePlatformConstants::PLATFORM_EVO)
            ->where('ip_address', $request_ip)
            ->exists();

        if (!$isAllowed) {

            $result = EVOProvider::ipNotAllowed($request_ip);
