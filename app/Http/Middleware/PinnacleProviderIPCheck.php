...
class PinnacleProviderIPCheck
{
    ...
    {
        // Log::info('PinnacleProviderIPCheck middleware');
        $web_request_service = new WebRequestService($request);

        $request_ip = $web_request_service->getIpAddress();

        $isAllowed = ProviderIPWhitelist::where('provider_code', GamePlatformConstants::PLATFORM_PINNACLE)
            ->where('ip_address', $request_ip)
            ->exists();
            
        if (!$isAllowed) {

            $result = PinnacleProvider::ipNotAllowed($request_ip);
