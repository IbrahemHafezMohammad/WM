...
class VIAProviderIPCheck
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        $isAllowed = ProviderIPWhitelist::where('provider_code', GamePlatformConstants::PLATFORM_VIA)
            ->where('ip_address', $request_ip)
            ->exists();

        if (!$isAllowed) {

            $result = VIAProvider::ipNotAllowed($request_ip);
