...
class SABAProviderIPCheck
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        $isAllowed = ProviderIPWhitelist::where('provider_code', GamePlatformConstants::PLATFORM_SABA)
            ->where('ip_address', $request_ip)
            ->exists();
            
        if (!$isAllowed) {

            $result = SABAProvider::ipNotAllowed($request_ip);
